...
3. 生成 `config\*.json`：桌面落盘路径按**当前用户**自动推导，端口沿用已有配置（首次 18080/18082）。口令沿用已有配置，首次安装时：门(18080) 随机生成 8 位并打印；剪贴板(18082) **留空** —— 第一次访问时在页面上自己设
...
| --- | --- | --- |
| `port` | 18082 | 监听端口 |
| `password` | 首次留空（设置模式） | 进入口令。**留空 = 第一次访问时在页面上自己设**，见下方「首次设置口令」 |
...
### 首次设置口令（`chat.config.json` 的 `password` 留空时）

剪贴板服务在**没有口令**时不再拒绝启动，而是进入**设置模式**：谁都还登录不了，
直到第一次访问的人把口令设出来。设完写回配置文件并立即生效，不用重启。

| 从哪访问 | 要什么 |
| --- | --- |
| **本机** `http://127.0.0.1:18082/` | 直接就是设置页，填两遍新口令即可 |
| **局域网** `http://<本机IP>:18082/` | 设置页会多要一个 6 位**一次性设置码**，它打印在本机日志里（`<安装目录>\logs\lan-chat.log`，形如「首次设置码：123456」） |

为什么局域网要多一道设置码：这个服务能在你的局域网里**往桌面写文件**。如果空着口令
等第一个访客来设，那么局域网里任何人都可以抢先把它设成自己的口令 —— 等于把机器送人。
设置码只存在于内存、设置成功后立刻失效，而它只打印在本机窗口里，所以能证明"你确实在
这台机器旁边"。人在机器前面时（127.0.0.1）就不需要它了。

口令要求：至少 4 位、两次一致、首尾无空格，且不能是历史默认值 `0322` 之类的常见口令。
设置模式期间 `/healthz` 会带 `"setup": true`，其余端点一律 503。

> 配对门（18080）**没有**这个流程，它的 `password` 必须非空；`install.ps1` 首次安装会
> 随机生成一个 8 位口令并打印出来。
...
| 打开 18082 只有"这台机器还没有口令" | 正常 —— 还没设口令，服务在设置模式。本机打开 `http://127.0.0.1:18082/` 直接设；从局域网设要填日志里的 6 位设置码。 |
| 18082 的 `/config` 等接口返回 503 | 同上，设置模式下的预期行为；设完口令即恢复。 |
...
* 已知取舍：明文 HTTP（局域网）；口令明文存在配置里。仓库里**没有任何可用的默认口令**：剪贴板服务首次安装是"空口令 + 第一次访问时自己设"，配对门是随机生成的 8 位。要更严可以换更长口令 + 收窄防火墙到单机 IP + 走 SSH 隧道。
* 首次设置口令的信任模型：本机（127.0.0.1）访问可直接设置；从局域网设置必须提供启动时打印在本机日志里的一次性设置码 —— 既不用你跑到机器前抄一个随机口令，也不会让局域网里任何人抢先把口令设成自己的。
